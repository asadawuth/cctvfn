...
export default function UserReportShopExcel() {
  ...
  const latesFileExcelReportShopUpdate = async () => {
    try {
      const response = await axios.post(
        `/servicesfileexcel/dataexcel-shoprequest/${data.start}/${data.end}`,
        null,
        {
          responseType: "blob",
      ...
      const opts = {
        suggestedName: `userReportshop_${data.start}_${data.end}.xlsx`, // ชื่อไฟล์แน่ะนำ
        types: [
          {
  ...
  return (
    <>
      ...
      <SeachDataForSaveExcel
        onChange={onChange}
        data={data}
        makeDataExcel={latesFileExcelReportShopUpdate}
      />
    </>
  );
}
