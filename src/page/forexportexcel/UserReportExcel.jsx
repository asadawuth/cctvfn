...
export default function UserReportExcel() {
  ...
  const latesFileExcelReportUpdate = async () => {
    try {
      const response = await axios.post(
        `/servicesfileexcel/dataexcel-userreport/${data.start}/${data.end}`,
        null,
        {
          responseType: "blob",
      ...
      const opts = {
        suggestedName: `userReports_${data.start}_${data.end}.xlsx`, // ชื่อไฟล์แน่ะนำ
        types: [
          {
  ...
  return (
    <>
      <HeaderMainMessage text="บันทึกข้อมูล Excel หัวข้อร้องเรียน" />
      <SeachDataForSaveExcel
        onChange={onChange}
        data={data}
        makeDataExcel={latesFileExcelReportUpdate}
      />
    </>
  );
}
