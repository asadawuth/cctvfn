export default function ModelForDeleteInShopId({
  shopId,
  commentId,
  onClose,
  ...
  const handleSummitForm = async (e) => {
    e.preventDefault();
    if (!shopId || !commentId) {
      console.error("Error: shopId or commentId is not defined");
      return;
    }
    try {
      await handleDeleteCommentId(shopId, commentId);
      onClose();
    } catch (error) {
  ...
  return (
    <>
      <form onSubmit={handleSummitForm} className="tw-flex tw-justify-center">
        <button
          type="submit"
