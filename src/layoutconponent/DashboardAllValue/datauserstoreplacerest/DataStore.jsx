...
export default function DataStore() {
  ...
  useEffect(() => {
    axios
      .get(`/aboutshop/datatshop?_page=${currentPage}`)
      .then((res) => {
  ...
  const handleDeleteShopId = async (datashopId) => {
    try {
      ...
      const res = await axios.get(`/aboutshop/datatshop?_page=${currentPage}`);
      setDataUserShop(res.data.shopList || []);
      setTotalPages(res.data.totalPages || 0);
    } catch (error) {
  ...
  return (
    <>
      <HeaderMainMessage text={"ข้อมูลร้านค้าของประชาชนในพื้นที่ ณ ปัจจุบัน"} />
      <Biglayout>
        <UserShopItem
