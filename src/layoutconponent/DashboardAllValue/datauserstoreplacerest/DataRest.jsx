...
export default function DataRest() {
  ...
  useEffect(() => {
    axios
      .get(`/aboutshop/datarest?_page=${currentPage}`)
      .then((res) => {
  ...
  const handleDeleteShopId = async (datashopId) => {
    try {
      ...
      const res = await axios.get(`/aboutshop/datarest?_page=${currentPage}`);
      setDataUserShop(res.data.shopList || []);
      setTotalPages(res.data.totalPages || 0);
    } catch (error) {
  ...
  return (
    <>
      <HeaderMainMessage text={"ข้อมูลแสดงสถานที่ในเขตราชาเทวะ ณ ปัจจุบัน"} />
      <Biglayout>
        <UserShopItem
